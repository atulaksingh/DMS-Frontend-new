...
export default function SalesCard({
        ...
        setInvoiceData([
          {
            ...
            invoice_type: response.data.sales_invoice.invoice_type || "",
          },
        ]);
      }
    } catch (error) {
  ...
  const handleSubmit = async (event) => {
    ...
    // if (rows.length === 0) {
    //   newErrors["rows"] = "At least one product is required";
    // } 
    // else {
    //   rows.forEach((row, index) => {
    //     if (!row.product) newErrors[`row_${index}_product`] = "Product is required";
    //     if (!row.hsnCode) newErrors[`row_${index}_hsn`] = "HSN Code is required";
    //     if (!row.unit) newErrors[`row_${index}_unit`] = "Unit is required";
    //     if (!row.rate) newErrors[`row_${index}_rate`] = "Rate is required";
    //   });
    // }
                  ...
                  <div>
                    ...
                    <div className="relative w-full">
                      <DatePicker
                        ...
                        selected={
                          invoiceData[0]?.po_date
                            ? isValid(parse(invoiceData[0].po_date, "dd-MM-yyyy", new Date()))
                      ...
                      <FaRegCalendarAlt
                        ...
                        onClick={() => po_date.current.setFocus()} // Focus the correct DatePicker
                      />
                    </div>
                  </div>
