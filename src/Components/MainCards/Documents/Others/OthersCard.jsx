...
export default function OthersCard({ rowId }) {
  ...
  const othersRules = {
    ...
    text: [{ test: (v) => v.length > 0, message: "Text is required" }],
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.status === 200 || response.status === 201) {
        ...
        // ✅ Wait until Redux updates before closing modal
        await dispatch(fetchClientDetails({ id, tabName: "Others" }));

        // ✅ Give slight delay for UI re-render
        setTimeout(() => {
          handleCreateClose();
        }, 300);
  ...
  const handleDeleteID = async () => {
    try {
      ...
      if (response.status === 200) {
        ...
        // ✅ Wait until Redux data updates before closing modal
        await dispatch(fetchClientDetails({ id, tabName: "Others" }));
